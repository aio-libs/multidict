...
def test_pickle(
    any_md_class: type[MultiDict[int]], pickle_protocol: int
) -> None:
    d = any_md_class([("a", 1), ("a", 2)])
    pbytes = pickle.dumps(d, pickle_protocol)
    obj = pickle.loads(pbytes)
    assert d == obj
    assert isinstance(obj, any_md_class)


def test_pickle_proxy(
    any_md_class: type[MultiDict[int]],
    any_md_proxy_class: type[MultiDictProxy[int]],
) -> None:
    d = any_md_class([("a", 1), ("a", 2)])
    proxy = any_md_proxy_class(d)
    with pytest.raises(TypeError):
        pickle.dumps(proxy)


def test_pickle_istr(
    ci_str_class: type[istr], pickle_protocol: int
) -> None:
    s = ci_str_class("str")
    pbytes = pickle.dumps(s, pickle_protocol)
    obj = pickle.loads(pbytes)
    assert s == obj
    assert isinstance(obj, ci_str_class)
...
def test_load_from_file(
    ...
    multidict_class_name = any_md_class.__name__
...
def test_load_istr_from_file(
    ...
    istr_class_name = ci_str_class.__name__
    pickle_file_basename = "-".join(
        (
