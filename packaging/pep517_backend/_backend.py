...
TRACING_CONFIG_SETTING = 'with-c-extension-tracing'
"""Config setting name toggle to include line tracing to C-exts."""

TRACING_ENV_VAR = 'MULTIDICT_EXTENSION_TRACING'
...
PURE_PYTHON_ENV_VAR = 'MULTIDICT_NO_EXTENSIONS'
"""Environment variable name toggle used to opt out of making C-exts."""
...
@contextmanager
def patched_dist_ext_modules(*, ext_modules):
    """Make `has_ext_modules` of `Distribution` always return `True`.

    :yields: None
    """
    _DistutilsDistribution.ext_modules = ext_modules
    try:
        yield
    finally:
        _DistutilsDistribution.ext_modules = None
...
def _in_temporary_directory(src_dir: Path) -> t.Iterator[None]:
    with TemporaryDirectory(prefix='.tmp-multidict-pep517-') as tmp_dir:
...
def maybe_prebuild_c_extensions(
    ...
    """
    line_tracing_requested = _include_cython_line_tracing(
        config_settings,
    ...
    if is_pure_python_build:
        ...
        if line_tracing_requested:
            _warn_that(
                f'The `{TRACING_CONFIG_SETTING !s}` setting requesting '
        ...
        with patched_dist_ext_modules(ext_modules=[]):
            yield
        return
    ...
    print(f'{line_tracing_requested=}')
    CFLAGS = ["-O2"]
    # if line_tracing_requested:
    #     # CFLAGS.extend(
    #     #     [
