...
COLD static int
_bitmap_remap(bitmap_t* bm, entry_t* oldentries, Py_ssize_t oldnentries,
              htkeys_t* newkeys, Py_ssize_t newcapacity)
{
    bitmap_t fresh;
    bitmap_init(&fresh, newkeys, newcapacity);
    if (bitmap_next(bm, 0) >= 0) {
        if (bitmap_reserve(&fresh) < 0) {
            return -1;
        }
        Py_ssize_t j = 0;
        for (Py_ssize_t i = 0; i < oldnentries; i++) {
            ...
            if (bitmap_test(bm, i)) {
                bitmap_set(&fresh, j);  // can't fail, reserved above
            }
            j++;
        }
    }
