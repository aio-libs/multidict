                                ...
                                PyObject **pvalue)
{
    ...
    *pidentity = ht_calc_identity(self->md, key);
    Py_DECREF(key);
    if (*pidentity == NULL) {
        if (pkey != NULL) {
            Py_CLEAR(*pkey);
...
multidict_itemsview_and1(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((arg = PyIter_Next(iter))) {
        ...
        if (ht_init_finder(self->md, identity, &finder) < 0) {
...
multidict_itemsview_or2(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    ht_init_pos(self->md, &pos);

    while (true) {
        int tmp = ht_next(self->md, &pos, &identity, &key, &value);
        if (tmp < 0) {
            goto fail;
        } else if (tmp == 0) {
            break;
...
{
    _Multidict_ViewObject *mv = PyObject_GC_New(
        _Multidict_ViewObject, md->state->KeysViewType);
    if (mv == NULL) {
        return NULL;
    }
...
multidict_keysview_repr(_Multidict_ViewObject *self)
{
    ...
    PyObject *ret = ht_repr(self->md, name, true, false);
    Py_CLEAR(name);
    return ret;
}
...
multidict_keysview_and1(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((key = PyIter_Next(iter))) {
        ...
        int tmp = ht_contains(self->md, key, &key2);
        if (tmp < 0) {
            goto fail;
        }
        if (tmp > 0) {
...
multidict_keysview_and2(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((key = PyIter_Next(iter))) {
        ...
        int tmp = ht_contains(self->md, key, NULL);
        if (tmp < 0) {
            goto fail;
        }
        if (tmp > 0) {
...
multidict_keysview_or2(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((key = PyIter_Next(iter))) {
        ...
        identity = ht_calc_identity(self->md, key);
        if (identity == NULL) {
            goto fail;
        }
    ...
    while (true) {
        int tmp = ht_next(self->md, &pos, &identity, &key, NULL);
        if (tmp < 0) {
            goto fail;
        } else if (tmp == 0) {
            break;
...
multidict_keysview_sub1(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((key = PyIter_Next(iter))) {
        ...
        tmp = ht_contains(self->md, key, &key2);
        if (tmp < 0) {
            goto fail;
        }
        if (tmp > 0) {
...
multidict_keysview_sub2(_Multidict_ViewObject *self, PyObject *other)
{
    ...
    while ((key = PyIter_Next(iter))) {
        ...
        tmp = ht_contains(self->md, key, NULL);
        if (tmp < 0) {
            goto fail;
        }
        if (tmp > 0) {
...
{
    return ht_contains(self->md, key, NULL);
}
...
{
    _Multidict_ViewObject *mv = PyObject_GC_New(
        _Multidict_ViewObject, md->state->ValuesViewType);
    if (mv == NULL) {
        return NULL;
    }
...
multidict_valuesview_repr(_Multidict_ViewObject *self)
{
    ...
    PyObject *ret = ht_repr(self->md, name, false, true);
    Py_ReprLeave((PyObject *)self);
    Py_CLEAR(name);
    return ret;
}
